...
def parse_alienvault_response(result: dict, artifact: str):
    """
    Deprecated function for parsing results
    but kept here for now for reference.
    """
    ...
    try:
        ...
        adversary: list = result.get("pulse_info", {}).get("related", {}).get("alienvault", {}).get("adversary", [])
        pulses: list = result.get("pulse_info", {}).get("pulses", [])
        ...
        link = f"https://otx.alienvault.com/browse/global/pulses?q={quote(artifact)}"
        return {
            "count": count,
            "pulses": pulse_data,
        ...
        logger.error("Error querying OTX AlienVault for '%s': %s", artifact, e, exc_info=True)
        return None
