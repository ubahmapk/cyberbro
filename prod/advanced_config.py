...
workers_count = secrets.get("supervisord_workers_count") or os.getenv(
    "SUPERVISORD_WORKERS_COUNT"
)
threads_count = secrets.get("supervisord_threads_count") or os.getenv(
    "SUPERVISORD_THREADS_COUNT"
)
timeout = secrets.get("supervisord_timeout") or os.getenv("SUPERVISORD_TIMEOUT")

if workers_count:
    config["program:cyberbro"]["command"] = config["program:cyberbro"][
        "command"
    ].replace(
...
if timeout:
    config["program:cyberbro"]["command"] = config["program:cyberbro"][
        ...
        "--timeout "
        + config["program:cyberbro"]["command"].split("--timeout ")[1].split()[0],
        f"--timeout {timeout}",
    )
    supervisor_conf_edited = True
