...
def url_observable():
    """Full URL observable."""
    return Observable(
        value="https://subdomain.example.com/path?query=value", type=ObservableType.URL
    )
...
def md5_observable():
    """MD5 hash observable."""
    return Observable(value="d41d8cd98f00b204e9800998ecf8427e", type=ObservableType.MD5)
...
def test_analyze_url_observable_success(success_response_with_data):
    ...
    result = engine.analyze(
        Observable(value="https://subdomain.example.com/path?query=value", type=ObservableType.URL)
    )
...
def test_url_domain_extraction_simple(success_response_with_data):
    ...
    responses.add(responses.GET, url, json=success_response_with_data, status=200)
    result = engine.analyze(Observable(value="https://example.com/path", type=ObservableType.URL))
...
def test_url_domain_extraction_with_port(success_response_with_data):
    ...
    responses.add(responses.GET, url, json=success_response_with_data, status=200)
    result = engine.analyze(
        Observable(value="http://example.com:8080/path", type=ObservableType.URL)
    )
...
def test_query_field_sha1_uses_files_sha1(sha1_observable):
    ...
    responses.add(responses.GET, url, json={"results": [], "total": 0}, status=200)
    engine.analyze(sha1_observable)
...
def test_query_field_sha256_uses_files_sha256(sha256_observable):
    ...
    responses.add(responses.GET, url, json={"results": [], "total": 0}, status=200)
    engine.analyze(sha256_observable)
...
def test_url_with_fragment(ipv4_observable):
    ...
    responses.add(responses.GET, url, json={"results": [], "total": 0}, status=200)
    result = engine.analyze(
        Observable(value="https://example.com/page#section", type=ObservableType.URL)
    )
...
def test_url_with_query_string(ipv4_observable):
    ...
    responses.add(responses.GET, url, json={"results": [], "total": 0}, status=200)
    result = engine.analyze(
        Observable(value="https://example.com/page?param=value&other=data", type=ObservableType.URL)
    )
